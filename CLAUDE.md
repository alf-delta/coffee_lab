...
- `BatchCard` обёрнут в `forwardRef` и пробрасывает `ref` на корневой `motion.article`:
  список рендерится через `AnimatePresence mode="popLayout"`, которому нужен ref для
  фиксации позиции при exit-анимации. Перепишешь без `forwardRef` — сломается анимация
  удаления/перестановки карточек.
